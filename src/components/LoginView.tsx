...
export default function LoginView({ 
  ...
  const handleSendOTP = (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName.trim()) {
      setErrorText(role === 'driver' ? 'Please enter your Driver ID.' : 'Please enter your name.');
      return;
    }
      ...
      if (enteredOTP === generatedOTP || enteredOTP === '1234') {
        ...
        if (role === 'rider') {
          ...
          const isRegistered = drivers.some(d => {
            ...
            return cleanD === cleanP || d.id.toLowerCase() === fullName.toLowerCase() || d.name.toLowerCase() === fullName.toLowerCase();
          });
  ...
  const handleConfirmLocation = async (e: React.FormEvent) => {
    ...
    const matchedDriver = drivers.find(d => d.id.toLowerCase() === fullName.trim().toLowerCase() || d.name.toLowerCase() === fullName.trim().toLowerCase());
    const resolvedName = matchedDriver ? matchedDriver.name : fullName;
    ...
    try {
      ...
      try {
        ...
        if (!exists) {
          accounts.push({
            name: resolvedName,
            phone: formattedPhone,
            email: `${resolvedName.toLowerCase().replace(/\s+/g, '')}@zipride.com`,
            role: 'driver',
            password: password,
            loginMethod: 'mobile'
          });
      ...
      window.scrollTo({ top: 0, left: 0, behavior: "instant" });
      triggerSuccessTransition(resolvedName, 'driver', formattedPhone);
  ...
  const handleSkipAnimation = () => {
    ...
    if (phase === 'riding-off' || phase === 'success-dissolve' || phase === 'passenger-walk' || phase === 'passenger-mount') {
      if (successData) {
        ...
        const matchedDrv = role === 'driver' ? drivers.find(d => d.id.toLowerCase() === fullName.trim().toLowerCase() || d.name.toLowerCase() === fullName.trim().toLowerCase()) : null;
        const finalResolvedName = matchedDrv ? matchedDrv.name : fullName;
        onLoginSuccess(finalResolvedName, role, phoneNumber);
      }
    } else {
      setPhase('hologram-active');
    }
              ...
              <div className="space-y-6">
                ...
                <form onSubmit={handleSendOTP} className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {/* Full Name / Driver ID */}
                    <div>
                      <label className="block text-[10px] font-bold tracking-wider text-theme-text-secondary mb-1.5 uppercase">
                        {role === 'driver' ? 'Driver ID' : 'Full Name'}
                      </label>
                      <motion.input
                        type="text"
                        value={fullName}
                        onChange={(e) => setFullName(e.target.value)}
                        placeholder={role === 'driver' ? 'e.g. DRV-2038' : 'Your name'}
                        whileFocus={{ scale: 1.01, boxShadow: "0 0 15px rgba(0, 200, 150, 0.25)" }}
