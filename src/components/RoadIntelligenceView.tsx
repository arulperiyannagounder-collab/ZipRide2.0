...
export const RoadIntelligenceView: React.FC = () => {
  ...
  const getHazardBadgeColor = (hazardType: string) => {
    switch (hazardType) {
      case 'Flood': return 'bg-blue-500/20 text-blue-300 border-blue-500/30';
      case 'Accident': return 'bg-rose-500/20 text-rose-300 border-rose-500/30';
      case 'Pothole': return 'bg-amber-500/20 text-amber-300 border-amber-500/30';
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400">Crowdsourced transit hazard reporting and safety telemetry</p>
        </div>

        {/* Road Health Score Card */}
        <div className={`flex items-center gap-4 border px-4 py-2.5 rounded-xl ${getScoreColor(roadHealthScore)}`}>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold opacity-80">Local Road Health Score</p>
            <h3 className="text-2xl font-black">{roadHealthScore}%</h3>
          </div>
          <div className="text-xs font-semibold px-2 py-0.5 rounded-md bg-black/20">
        ...
        <div className="lg:col-span-1">
          {!showAddForm ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all duration-200"
            >
              <Plus className="h-5 w-5" />
              Report Road Hazard
            </button>
          ) : (
            <form onSubmit={handleSubmit} className="bg-slate-800/40 border border-slate-800 p-5 rounded-xl space-y-4">
              <h3 className="text-sm font-bold text-white">File Hazard Report</h3>
              
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Hazard Type</label>
                <select
                  value={type}
                  onChange={(e) => setType(e.target.value as HazardReport['type'])}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg text-slate-300 p-2.5 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg text-slate-300 p-2.5 text-xs h-24 focus:ring-1 focus:ring-indigo-500 focus:outline-none resize-none"
                />
              </div>
              ...
              <div className="flex gap-2.5 pt-2">
                <button
                  ...
                  className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold py-2 px-3 rounded-lg border border-slate-700"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold py-2 px-3 rounded-lg shadow"
                >
                  Submit Report
                </button>
              </div>
          ...
          {/* Guidelines info */}
          <div className="mt-4 bg-slate-800/20 border border-slate-800 p-4 rounded-xl text-xs space-y-2 text-slate-400">
            <h4 className="font-bold text-slate-300">How verification works</h4>
            ...
            {hazards.length > 0 ? (
              hazards.map((hazard) => {
                ...
                return (
                  <div 
                    key={hazard.id} 
                    className="bg-slate-800/30 border border-slate-800/80 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-800/50 transition-colors"
                  >
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        {hazard.isVerified ? (
                          ...
                          <span className="bg-slate-500/10 text-slate-400 border border-slate-700/50 text-[10px] font-bold px-2 py-0.5 rounded-full">
                            Pending ({hazard.confirmations.length}/2 verifies)
                          </span>
                        )}
                      </div>
                      
                      <p className="text-xs font-semibold text-slate-200">{hazard.description}</p>
                      
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-slate-500">
                        <span className="flex items-center gap-1">
                          <MapPin className="h-3.5 w-3.5" /> Lat: {hazard.location.lat.toFixed(4)}, Lng: {hazard.location.lng.toFixed(4)}
                        </span>
                        <span className="flex items-center gap-1">
                          <Calendar className="h-3.5 w-3.5" /> {new Date(hazard.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
              ...
              <div className="text-center py-10 bg-slate-800/10 border border-dashed border-slate-800 rounded-xl text-slate-500 text-xs">
